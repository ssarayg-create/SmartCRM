...
export default function AuthView({ onLogin, initialMode = 'login' }: AuthViewProps) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Si la configuración es correcta, esto redirigirá al usuario a la página de Google.
      // No necesitamos hacer nada más aquí, ya que el redireccionamiento ocurre automáticamente.
    } catch (error: any) {
              ...
              <div className="space-y-6">
                ...
                <form onSubmit={showResetPassword ? handleResetPassword : handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    {errors.password && !showResetPassword && <p className="text-[10px] text-rose-500 font-bold ml-1">{errors.password}</p>}
                  </div>
                  ...
                  <Button type="submit" disabled={isLoading} className="w-full h-14 rounded-2xl bg-primary hover:bg-primary/90 text-white font-black text-lg shadow-xl shadow-primary/20 group">
                    {isLoading ? (
                      ...
                      <>
                        <span>{showResetPassword ? 'Enviar Enlace' : isLogin ? 'Entrar al Sistema' : 'Registrarme Ahora'}</span>
                  ...
                  <div className="text-center pt-4">
                    <p className="text-muted-foreground font-bold">
                      ...
                      >
                        {showResetPassword ? 'Volver al inicio' : isLogin ? 'Regístrate aquí' : 'Inicia sesión'}
                      </button>
                    </p>
                  </div>
                </form>
